...
- **9 大图表分区**：今日计数、卡片状态分布、记忆率、真实保留率、小时分布、间隔分布、难度分布、预测等，覆盖 FSRS 学习各维度；图表用纯 ArkUI（Text / Column / Row / Stack）绘制，不依赖第三方图表库。
- **偏好面板**：按需开关各图表分区，偏好持久化；图表配色自动跟随当前颜色主题。
...
- **学习统计**：调用 rslib `GraphsView` 接口获取图表统计，映射为每日热力强度与完成页信息，并在统计页以 9 大图表分区可视化呈现。
