...
为配合华为应用市场上架，2.3.3 发布包暂时隐藏了以下界面入口，但相关源码和自动化测试仍完整保留在仓库中：
...
这些功能只是隐藏入口，没有从源码中删除；发布开关集中在 `entry/src/main/ets/model/ReleaseFeatures.ets` 的 `SHOW_AI_AGENT_CHANNELS`。
...
### 学习时的 AI Agent 改卡（2.3.3 源码已包含，发布包暂时隐藏入口）
...
### AI 制卡与 AI 改卡（2.3.3 源码已包含，发布包暂时隐藏入口）
