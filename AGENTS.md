...
- 禁止“为了高内聚而拆文件”或“为了低耦合而加接口”；必须指出要降低的具体变更成本或风险
- 模块只承担一种业务变化原因：Provider 协议、循环编排、工具语义、策略/权限、草稿写入、历史持久化与 UI 展示不得混写
- 依赖保持单向：UI → Agent 编排 → 工具/策略边界 → 既有 Service 或写入执行器；下层不得反向 import 页面或组件
- 跨边界传递显式输入/输出类型，不用可变全局状态或 UI 状态充当模块协议
- 只有出现两个真实实现/调用方，或明确的测试替身、安全边界需求时才新增抽象；一次性路径保持直接
- 当前任务未暴露边界问题时不顺手重构；若一次改动被迫触碰 3 个以上无关模块，先检查职责归属再继续
- 架构调整必须说明“以后哪类改动会少改哪些位置”，不能只用设计模式名称作为理由
