...
test('card Agent tools register the four read and three ordinary draft tools', () => {
  for (const name of [
    'get_note_type_capabilities', 'get_note_context', 'search_cards', 'list_decks',
  ]) {
    assert.match(source, new RegExp(`registerRead\\('${name}'`));
  }
  for (const name of ['propose_create_notes', 'propose_update_notes', 'propose_move_cards']) {
    assert.match(source, new RegExp(`registerDraft\\('${name}'`));
  }
});

test('read context uses official services and registers discovered stable IDs', () => {
  ...
  assert.match(source, /registerCardIds/);
  assert.match(source, /registerNoteIds/);
  assert.match(source, /registerDeckIds/);
  assert.match(source, /registerNotetypeIds/);
});
