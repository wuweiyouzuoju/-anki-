...
test('encodeGetAddonInfoRequest matches ProtoWriter.writePackedInt64 for positive values', () => {
  // 验证手写 packed uint32 与 ProtoWriter.writePackedInt64 编码一致
  const ref = new ProtoWriter();
  ref.writeVarint(1, 26);
  ref.writePackedInt64(2, [123456, 7890]);
  const bytes = encodeGetAddonInfoRequest({ clientVersion: 26, addonIds: [123456, 7890] });
  assert.equal(hex(bytes), hex(ref.toBytes()));
});
...
test('decodeAddonInfo reads all four fields', () => {
  ...
  w.writeVarint(1, 100);       // id
  w.writeInt64(2, 1700000000); // modified
  w.writeVarint(3, 23);        // min_version
  w.writeVarint(4, 26);        // max_version
  const info = decodeAddonInfo(w.toBytes());
...
test('decodeAddonInfo skips unknown field (forward compatibility)', () => {
  ...
  // value = 0x01
  w.writeRawBytes(new Uint8Array([0x98, 0x06, 0x01]));
  w.writeVarint(4, 26); // max_version
  const info = decodeAddonInfo(w.toBytes());
  assert.equal(info.id, 5);
  assert.equal(info.maxVersion, 26);
...
test('decodeGetAddonInfoResponse reads multiple addon entries', () => {
  const addon1 = new ProtoWriter();
  addon1.writeVarint(1, 100);  // id
  addon1.writeVarint(3, 23);   // min_version

  const addon2 = new ProtoWriter();
  addon2.writeVarint(1, 200);  // id (varint = c8 01)
  addon2.writeVarint(4, 26);  // max_version

  const w = new ProtoWriter();
  w.writeMessage(1, addon1);
  w.writeMessage(1, addon2);

  const resp = decodeGetAddonInfoResponse(w.toBytes());
...
test('decodeCheckForUpdateResponse reads all fields', () => {
  const w = new ProtoWriter();
  w.writeString(1, '2.1.50');        // new_version
  w.writeInt64(2, 1700000000);       // current_time
  w.writeString(3, 'new release');   // message
  w.writeVarint(4, 99);              // last_message_id
  const resp = decodeCheckForUpdateResponse(w.toBytes());
...
test('decodeCheckForUpdateResponse skips unknown fields', () => {
  ...
  w.writeRawBytes(new Uint8Array([0x92, 0x03, 0x01, 0x78]));
  w.writeVarint(4, 7);
  const resp = decodeCheckForUpdateResponse(w.toBytes());
